<template>
  ...
  >
    ...
    <div class="flex flex-col gap-2">
      ...
      <InputText v-model="address" placeholder="Address" />
    </div>

    <template #footer>
      <Button class="bg-gray-500 hover:bg-gray-600" label="Cancel" severity="secondary" @click="close" />
      <Button class="bg-brand submit-btn" :loading="loadingSubmit" :disabled="!readyToSubmit" label="Submit" @click="submit" />
    </template>
  </Dialog>
...
const readyToSubmit = computed(() => name.value && phone.value && address.value)
